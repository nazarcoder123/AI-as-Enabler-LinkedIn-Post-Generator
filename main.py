...
#     # Log the start of the generation process with the perspective
#     logger.info(f"Generating LinkedIn post for perspective: {perspective}")
#     try:
#         # Initialize the Gemini model (consider making model name configurable)
#         model = genai.GenerativeModel("gemini-2.0-flash") # Using a commonly available model

#         # First, generate the LinkedIn post
#         # Log the construction of the prompt (DEBUG level for potentially large content)
#         logger.debug("Constructing prompt for post generation.")
#         prompt_post = (
#             f"You are a professional content strategist. Write a LinkedIn post "
#             f"based on the following article. The post MUST BE MAXIMUM 250 WORDS. DO NOT EXCEED 250 WORDS. "
...
#             f"Article Content:\n{article_text}"
#         )
#         # Log the API call to generate the post
#         logger.info("Calling Gemini API for post generation.")
#         post_response = model.generate_content(prompt_post)
#         # TODO: Add more robust error handling for API response content if needed
#         linkedin_post = post_response.text.strip()
#         # Log successful post generation
#         logger.info("Successfully generated LinkedIn post content.")

#         # Now, evaluate confidence of alignment with "AI as enabler"
#         # Log the construction of the confidence prompt (DEBUG level)
#         logger.debug("Constructing prompt for confidence score.")
#         prompt_confidence = (
...
#         # Count words to verify we're within the limit
#         word_count = len(linkedin_post.split())
#         logger.debug(f"Initial generated post word count: {word_count}")

#         # If word count exceeds 250, truncate to 250 words
#         if word_count > 250:
#             # Log that truncation is occurring
#             logger.warning(f"Generated post word count ({word_count}) exceeds 250 limit. Truncating.")
#             words = linkedin_post.split()
#             linkedin_post = " ".join(words[:250])
#             word_count = 250 # Update word count after truncation

#         # Format response to include word count
#         return linkedin_post, score, word_count

#     except Exception as e:
#         # Log errors occurring during the Gemini API calls or processing
#         logger.error(f"Error during Gemini API call or processing: {e}", exc_info=True)
#         # Consider more specific exception handling for genai errors if available
